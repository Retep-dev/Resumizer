    ...
    <div className="card-panel p-4 sm:p-6 rounded-3xl space-y-6 w-full">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-6 pb-6 border-b border-slate-200">
        ...
        <div className="flex flex-col sm:flex-row items-center text-center sm:text-left space-y-3 sm:space-y-0 sm:space-x-5 w-full lg:w-auto">
          <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-2xl border-2 border-[#069494] bg-[#00F0FF]/15 flex flex-col items-center justify-center font-extrabold shrink-0 text-[#069494] shadow-sm">
        ...
        <div className="w-full lg:w-80 space-y-3">
          <div>
            ...
            <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200">
              <div className="h-full bg-[#00F0FF] rounded-full" style={{ width: `${atsScore.keyword_match_score}%` }}></div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200">
              <div className="h-full bg-[#00F0FF] rounded-full" style={{ width: `${atsScore.experience_match_score}%` }}></div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200">
              <div className="h-full bg-[#00F0FF] rounded-full" style={{ width: `${atsScore.formatting_score}%` }}></div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 rounded-2xl bg-pink-50 border border-pink-200">
          ...
          <div className="flex flex-wrap gap-1.5">
            {atsScore.missing_keywords?.map((kw, i) => (
              <span key={i} className="px-2.5 py-1 rounded-md text-[11px] sm:text-xs font-bold bg-[#FF69B4] text-white">
                {kw}
              </span>
            ))}
          </div>
